...
int main(int argc, char* argv[]) {
    ...
    for (int y = 0; y < lines.size(); y++) {
        for (int x = 0; x < lines[y].length(); x++) {
            ...
            std::vector<node*> backup = nodes[y][x].links;
            ...
            nodes[y][x].links = backup;

            if (height + 1 >= left) {
                // nodes[y][x].links.push_back(&nodes[y][x-1]);
                nodes[y][x-1].links.push_back(&nodes[y][x]);
                nodes[y][x].pos[0] = '<';
            }
            if (height + 1 >= up) {
                // nodes[y][x].links.push_back(&nodes[y-1][x]);
                nodes[y-1][x].links.push_back(&nodes[y][x]);
                nodes[y][x].pos[1] = '^';
            }
            if (height + 1 >= right) {
                // nodes[y][x].links.push_back(&nodes[y][x+1]);
    ...
    while (visited.empty() || !notVisited.empty()) {
        // for (int i = 0; i < height; i++) {
        //     for (int j = 0; j < width; j++) {
    ...
    for (auto node : visited) {
        if (node->height == 'a') {
            if (node->tentativeDistance < min) {
                min = node->tentativeDistance;
            }
        }
    }
    std::cout << "Shortest path to endpoint is: " << min << std::endl;
